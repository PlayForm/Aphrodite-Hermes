...
# `binaries` IS in the forbidden set: NO binaries ever ship inside the
# plugin tree (catalog review, PR 118488). The plugin downloads from the
# GitHub release into the canonical runtime home and validates against the
# in-tree checksum list (generated by Build.yml). A `binaries/` dir inside
# the plugin dir is therefore misplaced and is moved to the runtime home,
# like the other runtime-state files (config, ccr.db).
