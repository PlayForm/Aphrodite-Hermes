...
# The plugin tree carries SHA256SUMS.txt GENERATED BY Build.yml (consolidated
# from the per-target release assets; committed to the child submodule by
# Publish.yml) - the pinned tree is the checksum source of truth. When the
# in-tree file matches BIN_VERSION and has an entry for this target, it is
# used directly (no network needed for the sums). Otherwise the release's
# per-target SHA256SUMS-<target>.txt asset is fetched; a missing sums file
# (e.g. a release cut before this was added) degrades to a loud warning
# rather than a hard failure, so older tags remain installable.
SUMS_FILE="$(mktemp)"
SUMS_ASSET="SHA256SUMS-${TARGET}.txt"
SUMS_OK=0
IN_TREE_SUMS="${SCRIPT_DIR}/SHA256SUMS.txt"
trap 'rm -f "${SUMS_FILE}"' EXIT
IN_TREE_VERSION=""
if [[ -f "${IN_TREE_SUMS}" ]]; then
	IN_TREE_VERSION=$(grep '^BINARY_VERSION:' "${IN_TREE_SUMS}" | head -1 | awk -F': *' '{print $2}' | tr -d '[:space:]')
fi
# Extract this target's block from the in-tree sums (keyed by the asset
# filename, which is unique across targets). Used only when the in-tree file
# exists AND its BINARY_VERSION matches the one being downloaded - a pinned
# tree never silently validates an older release.
if [[ -n "${IN_TREE_VERSION}" && "${IN_TREE_VERSION}" == "${V}" ]]; then
	if awk -v want="${TARGET}:" '$1 == want { in_block=1; next } in_block && $1 != "" && $1 !~ /:$/ { print } in_block && $1 ~ /:$/ && $1 != want { exit }' "${IN_TREE_SUMS}" 2>/dev/null | grep -q .; then
		awk -v want="${TARGET}:" '$1 == want { in_block=1; next } in_block && $1 != "" && $1 !~ /:$/ { print } in_block && $1 ~ /:$/ && $1 != want { exit }' "${IN_TREE_SUMS}" 2>/dev/null > "${SUMS_FILE}"
		SUMS_OK=1
		\echo "  ✓ using in-tree ${IN_TREE_SUMS} (BINARY_VERSION ${IN_TREE_VERSION})"
	fi
fi
...
if [[ "$SUMS_OK" -eq 1 ]]; then
	\echo "  ✓ using checksums for ${SUMS_ASSET}"
else
