...
# Several <<<CCR:hash|type|size>>> markers may accumulate across turns. You

# decide WHEN to resolve them - resolution is your call, made from the

# marker's type and size, not a reflex.

# CORE POLICY:

- Don't fetch a marker the instant it appears. Let pending markers
  accumulate in your working context; accumulation is what enables batching.
- When several markers are pending, resolve them in one parallel batch
  (multiple aphrodite_retrieve calls in a single turn) once you have enough
  to act on. Prefer batch retrieval over one-at-a-time fetching.
- Read markers without fetching them: type tells you what kind of content,
  size tells you how large. Reason about structure (counts, types, sizes)
